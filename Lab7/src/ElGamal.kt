...
data class ECPoint(val x: BigInteger, val y: BigInteger) {
    ...
    fun negate(p: BigInteger): ECPoint {
        if (this == INFINITY) return INFINITY
        return ECPoint(this.x, p - this.y)
    }
    override fun toString(): String {
        return if (this == INFINITY) "Infinity" else "(${x.toString(16)}, ${y.toString(16)})"
    }
}
...
class ElGamal(private val curve: EllipticCurve) {
    ...
    fun generateKeyPair(): Pair<BigInteger, ECPoint> =
        with(curve) {
            var d: BigInteger
            do {
                d = BigInteger(q.bitLength(), random).mod(q)
            } while (d == BigInteger.ZERO)
            val Q = d * P
            Pair(d, Q)
        }

    fun encrypt(message: ByteArray, publicKey: ECPoint): Pair<ECPoint, ECPoint> =
        with(curve) {
            val hashBytes = hasher.hashBytes(message, 256)
            val h = BigInteger(1, hashBytes)
            val messagePoint = h * P
            var k: BigInteger
            do {
                k = BigInteger(q.bitLength(), random).mod(q)
            } while (k == BigInteger.ZERO)
            val c1 = k * P
            val c2 = messagePoint + (k * publicKey)
            Pair(c1, c2)
        }

    fun decrypt(privateKey: BigInteger, ciphertext: Pair<ECPoint, ECPoint>): ECPoint =
        with(curve) {
    ...
    fun verify(message: ByteArray, decryptedPoint: ECPoint): Boolean =
        with(curve) {
            val hashBytes = hasher.hashBytes(message, 256)
            val h = BigInteger(1, hashBytes)
            val messagePoint = h * P
            messagePoint == decryptedPoint
        }
}